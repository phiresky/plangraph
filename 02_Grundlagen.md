TODO: griechische zeichen (utf8x)
...
_IA_: m=σ, es ist $n=1, f=1$ ⇒ Beh.

Sei also m ≥ 1

_Fall 1:_ G enthalte einen Kreis

⇒ es existiert $l \in E$ so dass $G':= G - e = (V, E\\{l})$ ebenfalls zusammenhängend und l an zwei Facetten grenzt die zu einer Facette in G' werden.
...
Da $$ n=n_1 + n_2, m=m_1 + m_2 - 1 \implies n-m+f = n_1 + n_2 - m_1 - m_2 - 1 +1 = (n_1 - m_1 + 1)+(n_2-m_2 + 1) - 2 = 2$$

_Folgerungen:_

- \#Facetten ist für jede pl. Einbettung von G gleich
...
Dann ist für (für jede planare Einbettung) jede Facette ein Dreieck und jede Kante grenzt an genau zwei Facetten.

$$ 3 f = 2 m $$
= mit Euler
$$ 3 (2-n+m) = 6-3n+3m $$

_Lemma:_ Sei G pl. Graph mit mind 3 Knoten. $d_{max}$ bezeichne Maximalgrad in G, $n_i$ \#Knoten von Grad i.

Dann gilt: $$6n_0 + 5n_1 + 4n_2 + 3n_3 + 2n_4 + n_5 \geq n_7 + 2n_8 + 3n_9 + \dots + (d_{max}(G) - 6) * n_{d_{max}} + 12$$
...
_Folgerung:_ Jeder planare Graph enthält mind. einen Knoten v mit $d(w) \leq 5$.
...
$"\implies"$ klar, da $K_5$ und $K_{3,3}$ nicht planar.

$\impliedby$ Es ist also "nur" zu zeigen: Wenn G nicht planar, dann enthält G einen $K_5$ oder $K_{3,3}$ als Minor.
